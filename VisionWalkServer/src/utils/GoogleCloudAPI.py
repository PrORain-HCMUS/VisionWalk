...
class GoogleCloudAPI:
    ...
    def translate_and_tts(self, text: str) -> Optional[bytes]:
        """
        Translate English text to Vietnamese and convert to speech
        Args:
            text (str): English text to translate and convert
        Returns:
            Optional[bytes]: Audio content in bytes if successful, None otherwise
        """
        try:
            ...
            translated = self.translator.translate(clean_text, src='en', dest='vi')
            translated_text = translated.text
            
            # Convert to speech
            synthesis_input = texttospeech.SynthesisInput(
                ssml=self._create_ssml(translated_text)
            )
            ...
            print(f"Translate and TTS Error: {str(e)}")
            return None
